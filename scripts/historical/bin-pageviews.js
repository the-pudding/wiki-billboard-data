...
function init() {
    ...
    const files = fs.readdirSync('./output/people-pageviews/').filter(d => d.includes(".json"))

    const peopleWithData = files.map((file,index) => {
        console.log(index)
        const data = JSON.parse(fs.readFileSync(`./output/people-pageviews/${file}`,'utf-8'))
    ...
    peopleNestedMonth.forEach(d => {
        ...
        fs.writeFileSync(`./output/pageviews-binned--month/${d[0].article}.csv`,formatted)
    })

    peopleNestedWeek.forEach(d => {
        const formatted = d3.csvFormat(d)
        fs.writeFileSync(`./output/pageviews-binned--week/${d[0].article}.csv`,formatted)
    })
